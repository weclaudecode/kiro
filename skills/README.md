...
**What it does.** Configures GitLab Duo Code Review (the non-agentic reviewer) for a project by producing a tailored `.gitlab/duo/mr-review-instructions.yaml`. Works review-then-author: inventory the repo's languages, layout, conventions, and process markers (a `detect-stack.sh` helper seeds `fileFilters` globs), harvest the real review nits, then write scoped instruction groups — by language *and* by concern/process (migrations, feature flags, API, telemetry) — phrased as hints, with the `Ask:`/`Remind:` and conditional-scoping patterns. Ships an annotated starter template, stack- and process-specific example groups (Python/Lambda, Terraform/Terragrunt, GitLab CI, Kubernetes, TypeScript, process-and-architecture, a security baseline), a validator that lints schema/globs/mandate-phrasing, and references for the YAML schema, glob syntax, group/instance-level templates, the reviewer's guidance-not-policy limits, and techniques distilled from GitLab's own production instructions file.
