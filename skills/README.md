...
## superpowers (14 skills, vendored from obra/superpowers)

**What it does.** The [obra/superpowers](https://github.com/obra/superpowers) v6.2.0 methodology (MIT), vendored **verbatim** as 14 standalone kiro skills: `using-superpowers`, `brainstorming`, `writing-plans`, `executing-plans`, `subagent-driven-development`, `dispatching-parallel-agents`, `test-driven-development`, `systematic-debugging`, `requesting-code-review`, `receiving-code-review`, `verification-before-completion`, `using-git-worktrees`, `finishing-a-development-branch`, `writing-skills`. Each triggers on its own description and is invocable as `/skill-name`, exactly like the Claude Code plugin. Skill bodies are unmodified — the port is two artifacts alongside them: `skills/using-superpowers/references/kiro-tools.md` (action → kiro tool mapping) and `steering/superpowers-bootstrap.md` (the always-on session bootstrap, generated by `scripts/gen-superpowers-bootstrap.sh`). **Both are required** — without the bootstrap the skills sit inert on disk and never trigger.

**Compatibility.** What the port needed to behave the way it does on Claude Code — and what still differs — is in [`docs/superpowers-compatibility.md`](../docs/superpowers-compatibility.md).

**When to use.** Starting any non-trivial build/fix/refactor where process matters — refine and plan before coding, drive each change test-first, review against the plan, close the branch deliberately. Skip for throwaway prototypes or pure config edits. For authoring *kiro* skills prefer `skill-creator`; upstream `writing-skills` ships because other superpowers skills reference it.
